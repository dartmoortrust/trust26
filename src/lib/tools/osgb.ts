...
GT_WGS84.prototype.isIreland = function () {
	return (
		this.latitude > 51.2 && this.latitude < 55.73 && this.longitude > -12.2 && this.longitude < -4.8
	);
};
...
GT_WGS84.prototype.isIreland2 = function () {
	...
	for (var i = 0; i < points.length; i++) {
		...
		if ((points[i][1] < y && points[j][1] >= y) || (points[j][1] < y && points[i][1] >= y)) {
			if (
				points[i][0] +
					((y - points[i][1]) / (points[j][1] - points[i][1])) * (points[j][0] - points[i][0]) <
				x
			) {
				oddNodes = !oddNodes;
			}
...
GT_WGS84.prototype.getOSGB = function (uselevel2) {
	...
	if (this.isGreatBritain()) {
		...
		var x1 = GT_Math.Lat_Long_H_to_X(this.latitude, this.longitude, height, 6378137.0, 6356752.313);
		var y1 = GT_Math.Lat_Long_H_to_Y(this.latitude, this.longitude, height, 6378137.0, 6356752.313);
		var z1 = GT_Math.Lat_H_to_Z(this.latitude, height, 6378137.0, 6356752.313);
...
GT_Math.E_N_to_Lat = function (East, North, a, b, e0, n0, f0, PHI0, LAM0) {
	...
	var E_N_to_Lat =
		(180 / Pi) * (PHId - Math.pow(Et, 2) * VII + Math.pow(Et, 4) * VIII - (Et ^ 6) * IX);
...
GT_Math.E_N_to_Long = function (East, North, a, b, e0, n0, f0, PHI0, LAM0) {
	...
	var E_N_to_Long =
		(180 / Pi) *
		(RadLAM0 + Et * X - Math.pow(Et, 3) * XI + Math.pow(Et, 5) * XII - Math.pow(Et, 7) * XIIA);
...
export const os2ll = (gridref: string) => {
...
export const en2ll = (easting: number, northing: number) => {
	console.log(easting, northing);
	const osgb = new GT_OSGB();
	osgb.setGridCoordinates(northing, easting);
	const { latitude, longitude } = osgb.getWGS84();
	return { lat: latitude, lng: longitude };
};
